    ...
    <form action="{{ route('admin.role.update',$role) }}" method="post" class="form form-horizontal" id="form-admin-add">
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="{{ $role->name }}" placeholder="" id="adminName" name="name">
            </div>
        </div>
        <div class="row cl">
                    ...
                    @foreach($nodeData as $item)
                        <li style="padding-left: {{ $item['level']*20 }}px ">
                            <input type="checkbox" value="{{ $item['id'] }}" name="node_ids[]"
                                   ...
                                   @if($item['pid']==0) class="checkAll" @endif @if($item['pid']!=0) names="{{ $item['pid'] }}" @endif
                            >
                            {{ $item['html'] }}{{ $item['name'] }}
                        </li>
                    @endforeach
